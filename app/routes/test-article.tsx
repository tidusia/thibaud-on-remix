import React from "react";
import Nav from "../components/Nav";
import { Link } from "react-router-dom";
import { MetaFunction } from "remix";
import Component, {
  attributes,
  // filename,
} from "../content/blog/comprendre-differents-types-tests.md";
import { MetaDescriptor } from "@remix-run/server-runtime/routeModules";
import { yearsWorking } from "../data/hours-working";
import Footer from "../components/Footer";

export const meta: MetaFunction = () => {
  const title = attributes.title;
  const description = attributes.excerpt;

  const metas: MetaDescriptor = {
    title,
    description,
    "og:type": "article",
    "twitter:creator": "@thibaud_duthoit",
  };

  if (attributes.picture) metas["og:image"] = attributes.picture;

  return metas;
};

export default function TestArticle() {
  return (
    <div>
      <header className="max-w-screen-xl mx-auto">
        <Nav />
      </header>

      <article className="article-page mx-auto px-4 py-12 text-xl">
        <h1 className="article-main-title mt-0 mb-12 sm:my-12 break-words">
          {attributes.title}
        </h1>
        <h2 className="mb-4 text-gray-600">{attributes.excerpt}</h2>
        <p className="mb-16 sm:text-right text-base leading-5 text-gray-500">
          <time dateTime={attributes.date} className="my-0">
            {new Date(attributes.date).toLocaleDateString("fr-FR", {
              year: "numeric",
              month: "long",
              day: "numeric",
            })}
          </time>
          <span className="mx-1">&middot;</span>
          {attributes.timeReading} de lecture
        </p>

        {!!attributes.picture && (
          <figure className="mb-16">
            <img src={attributes.picture} alt={attributes.pictureAlt} />
            <figcaption className="article-figcaption text-center text-gray-600 mt-4">
              {/* TODO: render real anchor for credits */}
              {attributes.pictureAlt}
            </figcaption>
          </figure>
        )}

        <div className="article-content">
          <Component />
        </div>
      </article>

      <section className="py-12 bg-gray-50 overflow-hidden md:py-20 lg:py-24">
        <div className="relative max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8">
          <svg
            className="absolute top-full right-full transform translate-x-1/3 -translate-y-1/4 lg:translate-x-1/2 xl:-translate-y-1/2"
            width="404"
            height="404"
            fill="none"
            viewBox="0 0 404 404"
            role="img"
          >
            <defs>
              <pattern
                id="ad119f34-7694-4c31-947f-5c9d249b21f3"
                x="0"
                y="0"
                width="20"
                height="20"
                patternUnits="userSpaceOnUse"
              >
                <rect
                  x="0"
                  y="0"
                  width="4"
                  height="4"
                  className="text-gray-200"
                  fill="currentColor"
                />
              </pattern>
            </defs>
            <rect
              width="404"
              height="404"
              fill="url(#ad119f34-7694-4c31-947f-5c9d249b21f3)"
            />
          </svg>

          <div className="relative">
            <blockquote>
              <div className="max-w-3xl mx-auto text-center text-2xl leading-9 font-medium text-gray-900">
                <footer className="mb-8 md:flex md:items-center md:justify-center">
                  <img
                    className="mx-auto h-40 w-40 rounded-full"
                    src="/images/thibaud-duthoit.jpg"
                    alt="Profil de Thibaud Duthoit, développeur front-end spécialisé en React.js"
                  />
                </footer>
                <p>
                  Je m'appelle Thibaud Duthoit, et je suis développeur front-end
                  depuis {yearsWorking}&nbsp;ans, spécialisé en intégration et
                  en React.js.
                </p>
                <p className="mt-4">
                  Si vous cherchez un développeur passionné, je serais ravi de{" "}
                  <Link to="/contact" className="text-blue-500 underline">
                    prendre&nbsp;contact
                  </Link>{" "}
                  avec vous.
                </p>
              </div>
            </blockquote>
          </div>
        </div>
      </section>

      <Footer />
    </div>
  );
}
